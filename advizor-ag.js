...
*
*  ag - I have modified the original so you are able to set limits of what the bot will spend in each market.
*  You can also split buy and sell orders over whatever you have defined.
*  Defaults:
*  Market Limits: S3 & S4 - 0.05 BTC, S5 & S7 - 0.3BTC
*  Order spilt: Buy: 5, Sell: 1 or 5 if there are over 10GHS (-0.00000001 seperates order splits)
*  
*/
"use strict";
var strategy = {
    name : "Advizer - modified by ag",
...
function onTick(){
    ...
    miners.forEach(function(miner){
        if(miner.sell){
            //console.log(Number(Number(miner.mid_price)+Number(miner.net_btc_per_ghs_daily)).toFixed(8));
            ...
            if(qty > 0){
				// if there are more than 10GHS Split into ...
				// How many orders to create
				var NUMBEROFORDERS = 5;
				
                if(qty > 10){
                    var qty = miner.holdings.amount / NUMBEROFORDERS;
                    console.log("Sell "+miner.holdings.total+" of your "+miner.name+" "+" for around "+sellprice+" split over "+NUMBEROFORDERS+" giving you around "+selltotal+"BTC use it to buy more "+miners[0].name);
					
					// Loop for NUMBEROFORDERS
					var loopVal = 0;
					while (loopVal < NUMBEROFORDERS) {
						// change buy price so orders don't stack up
						var sellprice = sellprice-0.00000001;
						// execute order
            ...
            if(miner.buy){
				// set and check if you have enough BTC to buy miner
				...
				if(qty > 0){
					...
					// Set BTC Limits per market set to dust to skip (nasty: still polls market but gives balance cant be 0 error)
					if(miner.name == "S3"){
					    var MAXSPEND = 0.05;
					}else if(miner.name == "S4"){
						var MAXSPEND = 0.05;
					}else if(miner.name == "S5"){
						var MAXSPEND = 0.3;
					}else if(miner.name == "S7"){
						var MAXSPEND = 0.3;
					}else{
						// miner not recognised set to dust
						var MAXSPEND = 0.0000001;
					}
					
					// Cancel all open orders for market, saves creating 50+ buy orders and allows easy enforcement of MAXSPEND
                    hashnest.cancelAllOrders(miner);
					
					// Calculate how many to buy with MAXSPEND then divide by NUMBEROFORDERS
                    var qty = Math.round(MAXSPEND / Number(miner.best_ask)) / NUMBEROFORDERS;
                    ...
                    console.log("Buy "+miner.name+" (MARKET LIMIT:"+MAXSPEND+"BTC) at around "+buyprice+", split over "+NUMBEROFORDERS+" orders and the yield is "+miner.yield+" BTC daily for each BTC invested.");
					
					// Loop for NUMBEROFORDERS
					var loopVal = 0;
					while (loopVal < NUMBEROFORDERS) {
						// change buy price so orders don't stack up
						var buyprice = buyprice-0.00000001;
						// execute order
